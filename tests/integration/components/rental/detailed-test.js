...
module('Integration | Component | rental/detailed', function(hooks) {
  ...
  test('it renders a header with a share button', async function(assert) {
    ...
    assert.dom('.info h2').containsText('Grand Old Mansion');
    assert.dom('.info p').containsText('a nice place to stay near San Francisco');
    assert.dom('.info a.button').containsText('Share on Twitter');
  });
